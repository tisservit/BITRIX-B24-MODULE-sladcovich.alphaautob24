        ...
        <div class="<?= $tabContainerClassName ?>">
            <ul id="<?= htmlspecialcharsbx($tabMenuContainerId) ?>" class="crm-entity-section-tabs-container"
                ...
                foreach ($tabs as $tab) {
                    ...
                    <? // Закрываем лишние кнопки в сделках ?>
                    <? if ($tab['deal_tab_more'] !== 'Y'): ?>

                        <li data-tab-id="<?= htmlspecialcharsbx($tab['id']) ?>"
                            class="<?= implode(' ', $classNames) ?>">
                            <a id="<?= $tab['id'] ?>" class="crm-entity-section-tab-link"
                               href="#"><?= htmlspecialcharsbx($tab['name']) ?></a>
                        </li>

                    <? else: ?>

                        <? // Закрываем кнопку "Приложения / Маркет" в сделках ?>
                        <? $customDealSubMenu = true; ?>

                        <? // Добавить кнопку еще и в нее скрытые кнопки?>
                        <li data-tab-id="<?= htmlspecialcharsbx($tab['id']) ?>"
                            class="<?= implode(' ', $classNames) ?>" style="display: none">
                            <a id="<?= $tab['id'] ?>" class="crm-entity-section-tab-link"
                               href="#"><?= htmlspecialcharsbx($tab['name']) ?></a>
                        </li>
                        ...
                        $arSubMenuItems[] = [
                            'text' => htmlspecialcharsbx($tab['name']),
                            'href' => '#',
                            'className' => implode(' ', $classNames),
                            'onclick' => 'BX.fireEvent(BX("' . $tab['id'] . '"),"click");BX.fireEvent(BX("body"),"click");BX.fireEvent(BX("more_deal_custom"),"click");'
                        ];
                        ?>
                ...
                if ($arResult['REST_USE'] && $customDealSubMenu !== false) {
                    ?>

                    <li class="crm-entity-section-tab" style="display: none">
                        <a href="#" class="crm-entity-section-tab-link"
                           onclick="BX.rest.Marketplace.open(<?= \CUtil::PhpToJSObject($arResult['REST_PLACEMENT_CONFIG']) ?>);"
                           class="crm-entity-section-tab-link"><?= \Bitrix\Main\Localization\Loc::getMessage('CRM_ENT_DETAIL_REST_BUTTON') ?></a>
                    </li>

                    <?
                    $arSubMenuItems[] = [
                        'text' => \Bitrix\Main\Localization\Loc::getMessage('CRM_ENT_DETAIL_REST_BUTTON'),
                        'href' => '#',
                        'className' => implode(' ', $classNames),
                        'onclick' => 'BX.PreventDefault();BX.rest.Marketplace.open(' . \CUtil::PhpToJSObject($arResult['REST_PLACEMENT_CONFIG']) . ');BX.fireEvent(BX("more_deal_custom"),"click");'
                    ];
                }
                ...
                <? if ($customDealSubMenu === true): ?>
                    <li class="crm-entity-section-tab" style="position: absolute; right: 0px">
                        <a class="crm-entity-section-tab-link" href="#" id="more_deal_custom">
                            Еще
                        </a>
                    </li>
                <? endif; ?>
                ...
                <script>
                    BX.ready(function () {
                        BX.bind(BX('more_deal_custom'), 'click', function () {
                            BX.PopupMenu.show('demo-popup-menu', BX('more_deal_custom'), [

                                <?
                                foreach ($arSubMenuItems as $menuItem) {
                                    echo CUtil::PhpToJSObject($menuItem);
                                    echo ',';
                                }
                                ?>
